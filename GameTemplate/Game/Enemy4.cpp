...
*/
	...
	if (diff.Length() <= 1000.0f)
	{
		//ワールド座標に変換。
		//座標をエネミーの少し上に設定する。
		Vector4 worldPos = Vector4(m_position.x, m_position.y + 280.0f, m_position.z, 1.0f);
...
void Enemy4::Chase()
{
	...
	if (m_Enemy4State != enEnemy4State_Chase)
	{
		return;
	}
...
void Enemy4::Collision()
{
	...
	//当たり判定処理はしない。
	if (m_Enemy4State == enEnemy4State_ReceiveDamage ||
		m_Enemy4State == enEnemy4State_Down)
	{
		return;
	}
		...
		for (auto collision : collisions)
		{
			...
			if (collision->IsHit(m_charaCon))
			{
				...
				if (m_hp <= 0)
				{
					...
					//被ダメージステートに遷移する。
					m_Enemy4State = enEnemy4State_ReceiveDamage;
				}
				return;
			}
		}
...
{
	//攻撃ステートでないなら処理をしない。
	if (m_Enemy4State != enEnemy4State_Attack)
	{
		return;
	}
...
const bool Enemy4::SearchPlayer() const
{
	Vector3 diff = m_player->GetPosition() - m_position;
...
void Enemy4::MakeAttackCollision()
{
	...
	//ボックス状のコリジョンを作成する。
	collisionObject->CreateBox(m_position, Quaternion::Identity, Vector3(180.0f, 20.0f, 20.0f));
	collisionObject->SetWorldMatrix(matrix);
	collisionObject->SetName("enemy4_attack");
}

void Enemy4::MakeFireBall()
{
	...
	//術者をエネミーに設定する。
	fireBall->SetEnMagician(Magic::enMagician_Enemy4);
}

void Enemy4::ProcessCommonStateTransition()
{
	//各タイマーを初期化。
	m_idleTimer = 0.0f;
	m_chaseTimer = 0.0f;
	...
	if (SearchPlayer() == true)
	{
		...
		if (IsCanAttack() == true)
		{
			...
			{
				//攻撃ステートに遷移する。
				m_Enemy4State = enEnemy4State_Attack;
			...
			{
				//待機ステートに遷移する。
				m_Enemy4State = enEnemy4State_Idle;
				return;
			}
		...
		else
		{
			...
			{
				//追跡ステートに遷移する。
				m_Enemy4State = enEnemy4State_Chase;
				return;
			}
			else {
				//現在が魔法攻撃ステートなら。
				if (m_Enemy4State == enEnemy4State_MagicAttack)
				{
					//連続で魔法を撃たせないように。
					//追跡ステートに遷移する。
					m_Enemy4State = enEnemy4State_Chase;
				...
				{
					//魔法攻撃ステートに遷移する。
					m_Enemy4State = enEnemy4State_MagicAttack;
					return;
				}
			}
		}
...
void Enemy4::ProcessIdleStateTransition()
{
...
void Enemy4::ProcessWalkStateTransition()
{
	//他のステートに遷移する。
	ProcessCommonStateTransition();
}

void Enemy4::ProcessChaseStateTransition()
{
	//攻撃できる距離なら。
	if (IsCanAttack() == true)
	{
...
void Enemy4::ProcessAttackStateTransition()
{
	//攻撃アニメーションの再生が終わったら。
...
void Enemy4::ProcessMagicAttackStateTransition()
{
...
void Enemy4::ProcessReceiveDamageStateTransition()
{
...
void Enemy4::ProcessDownStateTransition()
{
	//ダウンアニメーションの再生が終わったら。
	if (m_modelRender.IsPlayingAnimation() == false)
	{
...
void Enemy4::ManageState()
{
	switch (m_Enemy4State)
	{
		//待機ステートの時。
	case enEnemy4State_Idle:
		//待機ステートのステート遷移処理。
		ProcessIdleStateTransition();
		break;
		//追跡ステートの時。
	case enEnemy4State_Chase:
		//追跡ステートのステート遷移処理。
		ProcessChaseStateTransition();
		break;
		//攻撃ステートの時。
	case enEnemy4State_Attack:
		//攻撃ステートのステート遷移処理。
		ProcessAttackStateTransition();
		break;
		//魔法攻撃ステートの時。
	case enEnemy4State_MagicAttack:
		//魔法攻撃ステートのステート遷移処理。
		ProcessMagicAttackStateTransition();
		break;
		//被ダメージステートの時。
	case enEnemy4State_ReceiveDamage:
		//被ダメージステートのステート遷移処理。
		ProcessReceiveDamageStateTransition();
		break;
		//ダウンステートの時。
	case enEnemy4State_Down:
		//ダウンステートのステート遷移処理。
...
void Enemy4::OnAnimationEvent(const wchar_t* clipName, const wchar_t* eventName)
{
	(void)clipName;
...
const bool Enemy4::IsCanAttack() const
{
	Vector3 diff = m_player->GetPosition() - m_position;
	//エネミーとプレイヤーの距離が近かったら。
